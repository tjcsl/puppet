class math {
    ...
    exec { 'mathematica':
        path => '/usr/bin/:/bin/',
        command => "sh -c 'cd /tmp/yaourt-tmp-root/;wget -nc https://aur.archlinux.org/packages/ma/mathematica/mathematica.tar.gz;tar -xf mathematica.tar.gz -C aur-mathematica;mv aur-mathematica/mathematica/* aur-mathematica/;cd aur-mathematica;makepkg -csir --asroot && cd .. '",
        unless => 'which math',
        timeout => 0,
