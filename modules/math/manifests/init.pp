class math {
    ...
    exec { 'mathematica':
        path => '/usr/bin/:/bin/',
        command => "sh -c 'cd /tmp/yaourt-tmp-root/;wget https://aur.archlinux.org/packages/ma/mathematica/mathematica.tar.gz;tar -xf mathematica.tar.gz -C aur-mathematica;mv aur-mathematica/mathematica/* aur-mathematica/;rm mathematica.tar.gz*;cd aur-mathematica;makepkg -csir --asroot && cd .. && rm -r aur-mathematica'",
        unless => 'which math',
        timeout => 0,
